...
void drawTeapot(){
    ...
    auto paint = [&](geo::mat4f trans_mat, TGAImage &out){
        geo::mat4f ry = geo::rotateY(-PI/4.0f*4.2f);
        geo::mat4f sx = geo::scale(1.5f);
        auto ry_r = geo::transpose(geo::inverse(ry*sx));
        ...
        for(int i=0;i<nface;i++){
            ...
            bool check = 1;
            for(int j=0;j<3;j++){
                geo::vec4f l = geo::normalized(lightPos-ry*sx*vert[j]);
                geo::vec4f v = geo::normalized(camera-ry*sx*vert[j]);
                geo::vec4f h = geo::normalized(v+l);
                float intensity = geo::dot(l,ry_r*norm[j]);
                if(intensity<0.f){
                    check = 0;
                    break;
                }
                geo::OARColorf ld = kd * light * intensity;
                geo::OARColorf ls = ks * light * std::pow(std::max(0.f,geo::dot(ry_r*norm[j],h)), 100.f);
                geo::OARColorf la = ka * geo::vec4f(.3f, .3f, .3f, 1.f);
                color[j] = geo::toOARColor(ld+ls+la);
                screen[j] = trans_mat * ry * sx * vert[j];
                screen[j] /= screen[j].w;//这一句非常重要
                screen[j] = view * screen[j];
            }
            if(check) ras::triangle(out,screen,color,zbuf.get());
        }
    };
