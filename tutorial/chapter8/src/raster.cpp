...
bool ras::trianglePhong(
    ...
    for(int x=minx;x<=maxx;x++){
        for(int y=miny;y<=maxy;y++){
            ...
            geo::vec4f norm = alpha_r*tcoords.norms[0] + beta_r*tcoords.norms[1] + gamma_r*tcoords.norms[2];
            geo::vec4f world =  alpha_r*tcoords.worldCoords[0] + beta_r*tcoords.worldCoords[1] + gamma_r*tcoords.worldCoords[2];
            // 插值得出三角形内点的世界坐标与法向量
            ...
            geo::OARColorf ld = kd * light * std::max(0.f, geo::dot(l, norm)); // 有zbuffer后可以不用忽略
            geo::OARColorf ls = ks * light * std::pow(std::max(0.f,geo::dot(norm, h)), 100.f);
            geo::OARColorf la = ka * geo::vec4f(.3f, .3f, .3f, 1.f); //三种光强
            geo::vec4f intensity = ld + ls + la;
